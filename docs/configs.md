...
| `REDIS_USER_PASSWORD`         | password for specific user      | `""`    |
...
| `GATEWAY_UNKNOWN_REFERER_VALUE`  | unknown referrer value        | `unknown` |
...
| `USER_SERVICE_CACHE_EXPIRATION_SECONDS` | cache expiration (seconds)    | `3600`                            |
...
| `SHORTENER_EXPIRATION_SECONDS`              | expiration time (seconds)          | `500`                          |
| `SHORTENER_DEFAULT_LINK_EXPIRATION_MINUTES` | default link expiration (minutes)  | `1440`                         |
...
| `ANALYTICS_KAFKA_GROUP_ID`          | kafka consumer group id                                                                                 | `analytics-consumer`           |
...
| `ANALYTICS_TIMEZONE`                | Часовой пояс                                                                                            | `Europe/Moscow`                |
| `ANALYTICS_BATCH_SIZE`              | how many events are needed before sending them to ClickHouse                                            | `1`                            |
| `ANALYTICS_FLUSH_TIMEOUT`           | maximum time (sec) to wait before sending collected events to ClickHouse, even if the batch is not full | `3`                            |
| `ANALYTICS_MAX_RETRIES`             | maximum number of retry attempts                                                                        | `5`                            |
| `ANALYTICS_BASE_RETRY_DELAY_SECOND` | base delay between retries (sec)                                                                        | `1`                            |
...
| `BOT_LOCAL_SERVER_URL`        | the bot's local URL (for sending it in a message) | `http://localhost`    |
| `BOT_MAX_RETRIES`             | maximum number of retry attempts                  | `5`                   |
| `BOT_BASE_RETRY_DELAY_SECOND` | base delay between retries (sec)                  | `1`                   |
| `BOT_EXPIRATION_SECONDS`      | expiration time (sec)                             | `6000`                |
...
| `HTTP_PORT_RENDERER` | renderer port             | `8090`  |
...
| variable                                  | description                                  | example |
|-------------------------------------------|----------------------------------------------|---------|
| `GRPC_POOL_MAX_CONNECTIONS`               | maximum number of connections                | `100`   |
| `GRPC_POOL_MIN_CONNECTIONS`               | minimum number of connections                | `1`     |
| `GRPC_POOL_MAX_RETRIES`                   | maximum number of retry attempts             | `3`     |
| `GRPC_POOL_BASE_RETRY_DELAY_MILLISECONDS` | base delay between retries (ms)              | `300`   |
