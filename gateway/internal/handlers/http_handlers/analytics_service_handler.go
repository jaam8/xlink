...
func (h *AnalyticsServiceHandler) GetClicksByCountry(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
			Error(ctx.Context(), "couldn't get country stats",
	...
	logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
		Info(ctx.Context(), "got country stats",
...
func (h *AnalyticsServiceHandler) GetClicksByRegion(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
			Error(ctx.Context(), "couldn't get region stats",
				zap.Error(err), zap.String(handlers.UserIdKey, request.LinkOwner))
		return helpers.BadRequest(ctx, fmt.Sprintf("couldn't get region stats: %v", err))
	}

	logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
		Info(ctx.Context(), "got region stats",
			zap.String(handlers.UserIdKey, request.LinkOwner))
	return ctx.Status(fiber.StatusOK).JSON(response)
}
...
func (h *AnalyticsServiceHandler) GetClicksByBrowser(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
			Error(ctx.Context(), "couldn't get browser stats",
				zap.Error(err), zap.String(handlers.UserIdKey, request.LinkOwner))
		return helpers.BadRequest(ctx, fmt.Sprintf("couldn't get browser stats: %v", err))
	}

	logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
		Info(ctx.Context(), "got browser stats",
			zap.String(handlers.UserIdKey, request.LinkOwner))
	return ctx.Status(fiber.StatusOK).JSON(response)
}
...
func (h *AnalyticsServiceHandler) GetClicksByDeviceType(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
			Error(ctx.Context(), "couldn't get device type stats",
				zap.Error(err), zap.String(handlers.UserIdKey, request.LinkOwner))
		return helpers.BadRequest(ctx, fmt.Sprintf("couldn't get device type stats: %v", err))
	}

	logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
		Info(ctx.Context(), "got device type stats",
			zap.String(handlers.UserIdKey, request.LinkOwner))
	return ctx.Status(fiber.StatusOK).JSON(response)
}
...
func (h *AnalyticsServiceHandler) GetClicksByHour(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
			Error(ctx.Context(), "couldn't get per-hour stats",
				zap.Error(err), zap.String(handlers.UserIdKey, request.LinkOwner))
		return helpers.BadRequest(ctx, fmt.Sprintf("couldn't get per-hour stats: %v", err))
	}

	logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
		Info(ctx.Context(), "got per-hour stats",
			zap.String(handlers.UserIdKey, request.LinkOwner))
	return ctx.Status(fiber.StatusOK).JSON(response)
}
...
func (h *AnalyticsServiceHandler) GetClicksByDate(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
			Error(ctx.Context(), "couldn't get per-date stats",
				zap.Error(err), zap.String(handlers.UserIdKey, request.LinkOwner))
		return helpers.BadRequest(ctx, fmt.Sprintf("couldn't get per-date stats: %v", err))
	}

	logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
		Info(ctx.Context(), "got per-date stats",
			zap.String(handlers.UserIdKey, request.LinkOwner))
	return ctx.Status(fiber.StatusOK).JSON(response)
}
...
func (h *AnalyticsServiceHandler) GetClicksByReferrer(ctx *fiber.Ctx) error {
	...
	if err != nil {
		logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
			Error(ctx.Context(), "couldn't get referrer stats",
				zap.Error(err), zap.String(handlers.UserIdKey, request.LinkOwner))
		return helpers.BadRequest(ctx, fmt.Sprintf("couldn't get referrer stats: %v", err))
	}

	logger.GetOrCreateLoggerFromCtx(ctx.UserContext()).
		Info(ctx.Context(), "got referrer stats",
			zap.String(handlers.UserIdKey, request.LinkOwner))
	return ctx.Status(fiber.StatusOK).JSON(response)
}
